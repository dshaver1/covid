...
@Component
public class HealthcareWorkerExtractor extends AbstractExtractor implements Extractor<String, Map<String, Epicurve>> {
    ...
    public Optional<Map<String, Epicurve>> extract(List<String> raw, String id) {
        ...
        try {
            if (epicurveString.isPresent()) {
                ...
                List<HealthcareWorkerEpiPoint> tempList = getObjectMapper().readValue(epicurveString.get(), new TypeReference<List<HealthcareWorkerEpiPoint>>() {
                });
                Map<String, List<EpicurvePoint>> groupedByDate = tempList.stream().collect(Collectors.groupingBy(EpicurvePoint::getTestDate));
